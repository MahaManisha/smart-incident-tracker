...
export const getIncidentTrends = async (days = 30) => {
  const response = await axiosInstance.get(`/api/analytics/trends?days=${days}`);
  return response;
};

// Get incidents by severity
export const getIncidentsBySeverity = async () => {
  const response = await axiosInstance.get('/api/analytics/by-severity');
  return response;
};

// Get incidents by status
export const getIncidentsByStatus = async () => {
  const response = await axiosInstance.get('/api/analytics/by-status');
  return response;
};

// Get SLA compliance report
export const getSLACompliance = async (days = 30) => {
  const response = await axiosInstance.get(`/api/analytics/sla-compliance?days=${days}`);
  return response;
};

// Get responder performance
export const getResponderPerformance = async (days = 30) => {
  const response = await axiosInstance.get(`/api/analytics/responder-performance?days=${days}`);
  return response;
};

// Get average resolution time
export const getAverageResolutionTime = async (days = 30) => {
  const response = await axiosInstance.get(`/api/analytics/avg-resolution-time?days=${days}`);
  return response;
};

// Get team performance
export const getTeamPerformance = async () => {
  const response = await axiosInstance.get('/api/analytics/teams');
  return response;
};
...
export const exportReport = async (filters = {}) => {
  ...
  const response = await axiosInstance.post('/api/analytics/export', {
